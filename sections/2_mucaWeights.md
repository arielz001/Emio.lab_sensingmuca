...
1. **Array Parsing:** Access the parallel input arrays: `intensities` (containing the raw $v_i$ activation values) and `idx_list` (containing spatial matrix data indices).
...
3. **Centroid Equation with Dynamic Normalization:** Compute the absolute cumulative mass denominator $V_{sum}$ first, then calculate the normalized scalar weight factor $w_i$ inside your loop to accurately update the continuous tracking coordinate:
